...
os.environ.setdefault(
    "XDG_DATA_HOME",
    str(Path(__file__).resolve().parent / "voice_models" / "whispercpp_data"),
)
...
TEMP_DIR = Path("memory") / "voice_temp"
...
stt_model = Model(WHISPER_MODEL_NAME, print_progress=False, print_realtime=False, n_threads=WHISPER_THREADS)
...
def _transcribe(path: Path) -> str:
    segments = stt_model.transcribe(str(path), language="en", print_progress=False, print_realtime=False, no_context=True)
    parts = []
